    ...
    public class ButtonBrancher : MonoBehaviour
    {
            ...
            if(revealSettings.opening){
                ...
                switch(revealSettings.option)
                {
                    ...
                    case RevealSettings.RevealOption.Circular:
                        ...
                        {
                            case CircularSpawner.RevealStyle.SlideToPosition: RevealCircularlyNormal(); break;
        ...
        void RevealLinearlyFade()
        {
            //if(!isShown){
                for(int i = 0;i < buttons.Count;i++){
                    ...
                    if(i>0) previusButtonFader = buttons[i-1].GetComponent<ButtonFader>();
                    ...
                    if(previusButtonFader)
                    {
                        if(previusButtonFader.faded)
                        {
                            ...
                            else Debug.LogError("You want to fade your buttons, but they need a ButtonFader script to be attached first");
                        }
                    }
                    else
                    {
        ...
        void RevealCircularlyFade()
        {
            for (int i = 0; i < buttons.Count; i++)
            {
                ...
                if(i>0)previousButtonFader = buttons[i-1].GetComponent<ButtonFader>();
                else previousButtonFader = null;
                ButtonFader buttonFader = buttons[i].GetComponent<ButtonFader>();
                ...
                }else
                {
                    buttonRect.position = targetPos;
                    if(buttonFader)buttonFader.Fade(revealSettings.fadeSmooth);
                    else Debug.LogError("You want to fade your buttons, but they need a ButtonFader script to be attached fist");
                }
            } 
        }
